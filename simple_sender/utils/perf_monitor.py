...
def _sample_stats(values: list[float]) -> tuple[float | None, float | None]:
    if not values:
        return None, None
...
class AppPerformanceMonitor:
    ...
    def __init__(
        ...
        *,
        ...
        self._idle_cpu_samples: list[float] = []
        self._stream_cpu_samples: list[float] = []
    ...
    def _sample_once(self) -> None:
        ...
        with self._lock:
            ...
            if self._connected_and_idle():
                self._idle_cpu_samples.append(cpu_pct)
            if self._connected_and_streaming():
                self._stream_cpu_samples.append(cpu_pct)
    ...
    def _format_leak_watch(self) -> list[str]:
        ...
        for prev, cur in zip(self._snapshots[:-1], self._snapshots[1:]):
    ...
    def _build_report(self) -> str:
        ...
        idle_cpu_avg, idle_cpu_p95 = _sample_stats(self._idle_cpu_samples)
        stream_cpu_avg, stream_cpu_p95 = _sample_stats(self._stream_cpu_samples)
        lines: list[str] = []
