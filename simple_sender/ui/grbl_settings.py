...
class GRBLSettingsController:
    ...
    def save_changes(self):
        ...
        def worker():
            ...
            try:
                ...
                try:
                    self.app.after(0, lambda: self._finish_settings_save_failed(str(exc)))
                except Exception:
                    pass
                return
            ...
            try:
                self.app.after(
                    0,
                    lambda sent_count=sent: self._finish_settings_save(changes, sent_count),
                )
            except Exception as exc:
    ...
    def _finish_settings_save(self, changes, sent_count: int):
        self._settings_edited = {}
        self._settings_saving = False
        self._set_settings_edit_enabled(True)
        self._mark_settings_saved(changes, sent_count, refresh=True)

    def _finish_settings_save_failed(self, message: str | None = None):
        self._settings_saving = False
        self._set_settings_edit_enabled(True)
        if message:
            try:
    ...
    def _set_settings_edit_enabled(self, enabled: bool):
        if not enabled:
    ...
    def set_streaming_lock(self, locked: bool):
        if locked:
            self._set_settings_edit_enabled(False)
            return
