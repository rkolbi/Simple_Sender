...
from simple_sender.ui.autolevel_dialog_profiles import (
    _merge_auto_level_job_prefs,
    _select_auto_level_profile,
)
from simple_sender.ui.autolevel_dialog_helpers import (
    parse_float_var,
...
from simple_sender.ui.autolevel_dialog_io import (
...
def _log_split_result(log_fn: Callable[[str], None] | None, split_result) -> None:
    ...
    def _level_and_write(target_path: str) -> LevelFileResult:
        ...
        try:
            with open(target_path, "r", encoding="utf-8", errors="replace") as infile:
                ...
                split_result = split_gcode_lines_stream(
                    raw_iter,
        ...
        if split_result.failed_index is not None:
            ...
            error = _format_overlong_error(
                ...
                fallback_index=split_result.failed_index,
                fallback_len=split_result.failed_len,
                fallback_count=split_result.too_long,
                line_offset=header_count,
            )
            return LevelFileResult(None, 0, error, False)
        if split_result.modified_count == 0:
            return result
        ...
        return LevelFileResult(
            target_path,
            split_result.lines_written + header_count,
            None,
            False,
        )
...
def show_auto_level_dialog(app):
    ...
    avoidance_controls: list[ttk.Widget] = []
    ...
    def _parse_avoidance_areas() -> list[tuple[float, float, float]]:
        ...
        for idx, row in enumerate(avoidance_vars, start=1):
            ...
            def read_value(var: tk.StringVar, label: str) -> float:
                try:
                    value = float(var.get())
                except Exception as exc:
    ...
    for idx, row in enumerate(avoidance_vars, start=1):
        ...
        set_btn = ttk.Button(
            avoidance_frame,
            text="Read Position",
            command=lambda idx=idx - 1: _set_avoidance_from_position(idx),
        )
