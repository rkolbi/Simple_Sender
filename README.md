...
Current `3.20` local validation as of `2026-09-14` in the verified local Windows / Python `3.12.1` environment:
- Full pytest with coverage (`.\.venv\Scripts\python.exe -m pytest tests --cov=simple_sender --cov-report=xml --cov-report=term-missing`): `2738 passed, 1 skipped` (the skipped test is the environment-gated physical Kasa integration test), 73% total line coverage
...
The complete `run_tests.bat` wrapper passed all seven stages, including 89.4% aggregate critical-path coverage. Cross-platform CI, hardware-in-the-loop behavior, and Raspberry Pi image provenance/build validation were not run for this worktree. Historical validation snapshots remain in [CHANGELOG.md](CHANGELOG.md).
...
- Path View provides a passive 2D planned-path preview with a reported WPos marker. When no job is loaded, it can show the controller-configured Machine Travel Area from positive `$130/$131` travel settings. It can also list inferred tool-change sections for display-only selection. That rectangle is visualization context only, not a fixture boundary, stock boundary, verified soft-limit boundary, or safety constraint; tool-change inference is not authoritative controller state. Displayed travel area, preview parsing, rendering, warnings, markers, and failures do not participate in streaming admission, soft-limit enforcement, machine control, probing, recovery, or job-completion decisions. Large jobs are rendered with bounded batch/LOD geometry for Pi-friendly resource use; the Path View detail control selects a bounded source sample, and sampled previews are labeled approximate and are not complete toolpaths.
...
- Directives in streamed files (`VACUUM_ON`, `VACUUM_OFF`, `TC:<description>`, and uppercase `M6/M06 <description>`) are handled internally and never forwarded to GRBL. The `M6`/`M06` alias requires whitespace and a nonempty description; bare or controller-style uses remain rejected.
...
4) **Load G-code**
   - Click **Read Job** to open the shared OS file picker. On Linux, the app temporarily raises Tk dialog scaling, applies the current dialog theme, and enforces a minimum dialog size so the chooser stays readable under Pi/Openbox touchscreen setups. Linux dialogs default to `/root/CNC_Jobs` unless **App Settings > Theme > Linux File Dialog Default Path** is set to another valid folder; invalid paths fall back safely. The loader strips comments and blank lines into an application-owned file-backed snapshot, computes its SHA-256 digest, and validates every executable line before the job becomes runnable. Unicode is preserved in recognized `TC:<description>` and uppercase `M6/M06 <description>` directives and ignored inside comments; non-ASCII in executable G-code is rejected without character deletion. Overlong, system-command, bare or controller-style `M6`/`M06`, duplicate-word, conflicting-modal, or other GRBL-incompatible jobs are rejected during load.
...
5) **App safety options**
   ...
   - Auto-reconnect can reopen an idle/startup connection, but it does not resume a job interrupted by a USB blip. An active-job disconnect enters Recovery Required because GRBL may already have buffered commands beyond the last acknowledgment.
...
6) **Prepare the machine**
   ...
   - For dry runs, enable **Dry run: strip spindle/coolant/S/T from streamed G-code** in **App Settings > Safety**. Sender-side `TC:<description>` and uppercase `M6/M06 <description>` directives still run the built-in tool-change flow; bare or controller-style `M6`/`M06` is rejected during load.
...
7) **Start and monitor**
   ...
   - After confirmation, Run performs a background SHA-256 verification of the retained canonical snapshot handle. The UI remains responsive, no job command is admitted during verification, and the worker revalidates the exact source, handle, connection, stream, and recovery identities before streaming starts. Whole-job parsing and command validation are not repeated.
...
8) **Alarms / errors**
   ...
   - During an active job, including after the final acknowledgment while controller Idle or the exact UI completion acknowledgement is pending, a GRBL `error:`, unexpected startup banner/reset, or current-session connection loss enters **Recovery Required**. The sender atomically closes command admission, invalidates the stream/recovery epoch, quarantines outstanding acknowledgments and pending confirmed vacuum directives, records the uncertain transmitted-line range, and attempts feed hold followed by soft reset when possible. A queued completion from the retired epoch cannot run post-job actions, and events from a provably retired connection generation are discarded without altering the replacement session.
   - Recovery Required blocks normal Resume/Cycle Start, Resume From, manual G-code, macros, probing, and automatic reconnect-resume. Do not use `last_acked_index + 1` as a restart point: later commands may already have entered GRBL's RX buffer or executed.
   - Loading, replacing, restoring, and clearing a job uses a two-phase source transaction. The worker first reserves an immutable source identity that cannot stream. The matching filename, preview, metadata, line counts, and optional send cache must finish installing before the worker atomically commits that source as runnable. Stale callbacks and installation failures leave a cleared/unrunnable source instead of restoring an ambiguously installed older job.
   - Recovery progresses through **Reset Required**, **Reset Sent - Awaiting Banner**, and **Reset Confirmed - State Untrusted**. A matching startup banner proves only that GRBL reset; it does not restore Ready. Every additional banner retires all evidence gathered for the prior recovery. A replacement connection receives a new recovery identity and never reloads the interrupted job.
   - In the recovery dialog, synchronize complete `$G` and `$#` response transactions, home or explicitly accept the physically verified current position, verify an exact synchronized M5/M9 controller state against the physical spindle/coolant condition, and deliberately complete recovery. The worker binds motion, units, distance/arc-distance, plane, feed, WCS, TLO, spindle, coolant, F/S/T values, G92, offsets, and coordinates to that exact transaction. Every trusted XYZ report must contain exactly three finite numbers; duplicate `MPos`, `WPos`, or `WCO` fields are rejected even when equal, and simultaneous representations must agree with `MPos = WPos + WCO` within GRBL reporting-rounding tolerance. TLO must contain exactly one finite scalar. Missing/extra components, empty fields, trailing garbage, contradictions, `nan`, infinities, and overflow invalidate the owning evidence. Ordinary commands remain blocked until that exact immutable snapshot object is installed in the UI and macro caches and acknowledged with its exact single-use worker finalization identity. If installation fails, **Retry Recovery Finalization** reapplies only the same current approved object and identity while admission stays closed. Tool Setup/TLO-dependent workflows remain blocked until their own references are re-established.
   - The Recover action is forced visible and enabled whenever worker-owned recovery is active, even while ordinary toolbar controls are locked. Its identity-bound Disconnect action closes only that recovery session. Serial disconnect is **not** an emergency stop: buffered or currently executing controller work may continue, so use the physical emergency stop or power isolation when required. If reset transmission or confirmation fails, assume the machine may still be executing.
   - A first clean connection, or reconnect after a clean idle disconnect, follows a separate normal-session initialization path and does not enter execution recovery merely because a port opened. Startup auto-connect presents a welcome/communication popup while Simple Sender attempts controller communication. Startup-banner ownership is bound to the exact generation and serial object and commits one explicit outcome: pending, consumed, timed out, or retired. Status received first may update telemetry but cannot consume banner ownership or enable commands, and a timeout cannot tear down a session after its banner was consumed. Normal initialization gathers fresh `$G`, `$#`, position, conservative M5/M9 evidence, and a `$$` GRBL settings snapshot without inheriting prior-generation trust; after synchronization and settings capture, the readiness popup offers **Home Now** or **OK / I’ll Home Later**. **Home Later only dismisses the reminder; home the CNC before jogging, zeroing, probing, parking, running jobs, or other movement actions.** Job Ready is withheld until the exact immutable snapshot is installed and acknowledged. A replacement connection after unresolved execution uncertainty receives a new recovery identity, remains blocked, and does not restore the interrupted job.
...
2) Wait for the current-session GRBL banner (**Communication Ready**), controller synchronization, and automatic `$$` settings capture. The startup communication popup is replaced by a readiness popup with **Home Now** and **OK / I’ll Home Later** only after GRBL Settings has data. Use **Home Now** or the workflow-row **Home** button later to complete the exact snapshot handoff (**Job Ready**). Homing waits for current-session Home-to-Idle evidence. Treat the machine as not ready for movement until it has been homed; do not jog, zero, probe, park, run jobs, or perform machine-coordinate moves before homing, except for the homing action itself. A status frame alone is not the startup handshake.
...
## Core Behaviors
- **Handshake and Job Ready:** The exact current generation/serial startup owner has one terminal outcome and a consumed banner cannot later time out. Pre-banner status is telemetry only. Job-driving controls require fresh normal-session `$G`/`$#` and position/accessory trust, exact installation and worker acknowledgement of that immutable snapshot, plus an exact committed job-source identity; Communication Ready alone does not mean Run is allowed, and reconnect cannot reuse prior UI or macro cache values.
...
- **Auto-reconnect:** When idle and not user-disconnected, retries the last port with backoff and respects "Reconnect to last port on open". It is disabled after active-stream execution becomes uncertain and never resumes from the last acknowledged line.
- **Alarms:** ALARM:x, "[MSG:Reset to continue]", or status Alarm stop/clear queues, lock controls except Unlock/Home/ALL STOP.
- **Active-stream uncertainty:** A streamed GRBL `error:`, including an error after final ACK while Idle/UI completion acknowledgement is pending, an unexpected startup banner, current-session connection loss, disconnect before controller completion, or a safety-significant realtime write failure enters worker-owned Recovery Required state. A current-session jog-cancel write failure also fails closed even if the last visible controller state was Idle, because the sender cannot prove whether the cancel byte reached GRBL. Admission and serial writes are epoch-checked, pending confirmed vacuum directives and retired queue/ACK/UI completion work are quarantined, and reconnect is blocked while an old session thread can still report state. Send-time validation failures remain terminal stream errors but are distinct from controller execution uncertainty.
- **Controller-confirmed suspension:** Pause closes stream TX before sending `!` and stays Pause requested until current-session `Hold` evidence arrives. External Hold and every Door substate close both ordinary stream TX and exact-owner tool-change macro/probe/positioning TX without discarding ACK accounting. Door closure never auto-resumes. Explicit Resume sends `~` and keeps controller admission closed until current-session status proves Hold/Door ended. If a `TC:` workflow is pending, confirmation returns to the independent Tool Change pause rather than reporting ordinary Running; job lines remain blocked until that directive completes. Confirmation timeout enters Recovery Required.
- **Auto-Level ownership and cancellation:** Auto-Level acquires one exclusive worker-owned lease only when no stream, queued command, reserved RX slot, reset/synchronization, jog uncertainty, or other controller work conflicts. Every probe and modal command, fresh Idle/PRB result, callback, restoration, and map install is bound to that exact generation/serial/stream/recovery/source lease. After restoration, the worker issues one exact single-use installation ticket and keeps ordinary machine-driving controls blocked while the UI stages the result; only an exact final worker acknowledgement publishes the map and enables Apply/Save. Installed maps retain controller, source, coordinate-context, trust, workflow, and installation provenance and are invalidated by reset, alarm, recovery, disconnect/replacement session, source replacement, or relevant WCS/G92/TLO context changes. Historical map files load as inactive and cannot be applied or re-saved in this release. Cancel, dialog close, and Escape retire installation ownership and use the existing hold/reset recovery path when motion or modal state may be uncertain. Failed restoration or stale finalization cannot publish success. A software stop is not an emergency stop; keep physical E-stop and power isolation available.
- **Recovery synchronization:** `$G` and `$#` must each return the required values and terminal acknowledgment for one current recovery transaction. Report ownership is recognized from the known modal/status/parameter prefix before the full envelope is accepted; any malformed recognized report invalidates and clears its owning evidence, so a later `ok` cannot approve earlier values. Trusted XYZ reports require exactly three finite values, reject duplicate `MPos`/`WPos`/`WCO`, and require simultaneous coordinate representations to agree within reporting-rounding tolerance; TLO requires exactly one finite scalar. Repeated resets and replacement connections retire prior evidence. Admission reopens only after the exact approved snapshot object replaces stale UI/macro caches, the interrupted job is cleared, and the worker consumes its exact single-use finalization identity; a failed UI handoff remains locked and offers identity-bound **Retry Recovery Finalization**.
- **Accessory recovery dominance:** Kasa barriers, command ownership, and Recovery OFF tombstones use the stable physical device ID plus outlet index, so hostname, IP, device-ID-qualified aliases, cache replacement, and rediscovery of the same device share one safety domain. If the device ID is not known yet, unresolved aliases use a conservative provisional domain until resolution atomically merges ownership into the canonical outlet. The exact software dispatch commitment occurs while the canonical outlet barrier and router state lock are held, after the final ownership/dominance check and before the network API call. Physical outlet dispatch ownership is process-level, records a unique router-instance owner, and can outlive one router while a committed call is active; router shutdown drains only that exact instance, may report incomplete if its physical dispatch has not drained, and a replacement router serializes against unresolved same-outlet work before issuing conflicting commands. Recovery OFF installed before dispatch commitment suppresses the work. Recovery OFF installed afterward cannot cancel the committed call; it supersedes the logical result and remains required/retryable until OFF physically succeeds. `requested` and `failed_unknown` Recovery OFF states remain dominant across later recovery epochs, reconnects, and replacement routers; reconnect alone does not clear physical uncertainty. A successful identity-valid OFF retry must first become `confirmed`, and confirmed dominance is retired only after successful recovery completion for the exact identity with no active physical dispatch. Idle registry entries are cleaned up only when no router holds a live reference, no active commitment remains, and no unresolved or unretired tombstone remains. Neither `requested` nor `failed_unknown` proves physical OFF; verify the accessory manually. Kasa remains convenience automation, not a safety-rated system.
- **Ongoing coordinate telemetry:** After Job Ready, ordinary `MPos`, `WPos`, and `WCO` status evidence uses the same strict three-axis parser as initialization and recovery, with recognized coordinate field names normalized before installation. Raw status text is retained separately from the last successfully validated and installed coordinate signature. Duplicate fields, extra or missing axes, non-finite/overflow values, and inconsistent simultaneous representations cannot become a duplicate/freshness baseline, update coordinate caches, advance the position throttle, increment the coordinate sequence, or signal a fresh-coordinate waiter. Freshness is published only after those exact validated values have been installed in the operational, DRO, and macro coordinate caches.
- **Loaded-source ownership:** Job replacement, clear, and reconnect restore use reserve/install/commit transactions. Run and Resume From require the UI identity to exactly match a worker source in `COMMITTED` state and no pending installation. A reserved source is worker-unrunnable; only the exact identity whose UI/cache stages all completed can commit. Rejection, cancellation, recovery, disconnect, or installation failure retires the reservation into a cleared/unrunnable state, and callbacks from older UI load generations cannot mutate or re-enable the newer source.
...
- **Streaming:** Character-counting; uses Bf feedback to size the RX window; stops on error/alarm; buffer fill and TX throughput shown. Each line is counted with the trailing newline for buffer accounting. Executable lines are rejected if they exceed 80 bytes or contain non-ASCII characters; recognized `TC:<description>` and uppercase `M6/M06 <description>` payloads may contain Unicode and are intercepted before serial transmission.
...
- **Tool-change sender directive:** Lines that start with `TC:`, or uppercase `M6`/`M06` followed by whitespace and a nonempty description, are normalized to the same internal directive before admission. The description, including Unicode such as `°`, is preserved. These lines are intercepted before queue/send, treated as required-tool prompts, shown in the existing tool-change popup, then routed through the built-in Tool Change workflow. One opaque identity binds the exact connection, serial object, stream/recovery epochs, source, directive index, and request token to every tool-change command and completion callback; stale, reconstructed, duplicate, stopped, reset, or prior-session work cannot command or complete a later directive. The UI thread is associated with that same identity: if current Tool Change B arrives while a stale A thread is still alive, A is signaled and B is explicitly failed with a Stop/restart instruction instead of remaining silently pending. The supported production workflow assumes files generated by the bundled Simple-Sender Vectric posts, which continue to emit `TC:` with their stop-before-directive and restart-after-segment contract. Streaming stays paused with a scoped no-timeout override until the operator finishes that workflow, then resumes. After the built-in tool-change flow completes, the machine parks at safe Z over WCS `X0/Y0` and the posted job is expected to reposition from there. Sender-owned tool directives are marked handled and never sent to GRBL.
...
- Program pauses (`M0`/`M1`) pause after acknowledgement. Bare `M6`/`M06` and controller-style combinations such as `T2 M6` are rejected before admission. Uppercase `M6 <description>` and `M06 <description>` are sender-owned aliases for `TC:<description>`; lowercase forms are not recognized. Bundled Simple Sender/Vectric posts continue to emit `TC:`.
- Sender directives (`VACUUM_ON`, `VACUUM_OFF`, `TC:<description>`, and uppercase `M6/M06 <description>`) are intercepted before serial transmission, so they do not generate unknown-command GRBL errors.
- Console Save pre-fills a timestamped filename (`simple_sender_console_YYYYMMDD_HHMMSS.txt`) for touch-friendly export.

## GRBL Settings UI
- If the post-connect `$$` snapshot has already been captured, the first popup open uses that cached data immediately. Refresh $$ (idle, not alarmed, after Communication Ready and Job Ready) requests a newer dump; if Job Ready is still pending, the refresh stays queued instead of being lost. The table is scrollable, shows descriptions, supports inline numeric validation/ranges, and keeps pending edits highlighted until saved. If enabled, the optional Raw $$ popup button opens the raw text capture.
...
User macro launches are blocked while the controller is streaming, during alarms, or whenever the app disconnects, and they still respect Training Wheels confirmations. User macro buttons appear on the main screen only when a file-backed slot is actually assigned. Streamed `TC:<description>` and uppercase `M6/M06 <description>` directives pause the stream and reuse the protected built-in Tool Change workflow.
...
During streamed jobs, `TC:<description>` and uppercase `M6/M06 <description>` lines use this same protected tool-change path and intentionally wait with no timeout until the operator completes the tool-change flow. General macro timeout settings still apply to user macros.
...
- Recovery behavior: entering Recovery Required immediately submits dominant OFF requests for configured Kasa job accessories without waiting for UI event handling or the normal vacuum delay. Aliases for the same stable device ID and outlet share one canonical barrier and tombstone. Work not yet dispatch-committed is suppressed; an already committed network operation cannot be canceled and therefore still requires the dominant OFF result or retry. The app records OFF as requested, confirmed, or failed/unknown; issuing the request is not treated as physical confirmation, and stale accessory callbacks cannot replace the current recovery result.
...
- No $$: wait for Communication Ready; startup normally captures `$$` automatically before the readiness popup. Manual refresh requires the machine to be idle, not alarmed, and not homing or streaming.
...
- **Why can manual $$ refresh be deferred?** To avoid interleaving settings capture with startup synchronization, homing, streaming, alarms, or recovery. On a clean startup, Simple Sender requests `$$` automatically before the readiness popup.
...
| Built-in: Tool Change | Re-probes after a tool swap, reapplies the stored reference tool height, then parks at safe Z over WCS `X0/Y0`; this is also the workflow used by streamed `TC:<description>` and uppercase `M6/M06 <description>` directives. | Tool changes after a reference tool has already been captured by Job Setup in the current valid session. | Implemented directly in code, uses the shared fixed-sensor settings, runs with built-in unlimited-wait workflow handling, and expects the posted job to reposition after the park move. |
...
- Refresh $$: requests a fresh `$$` dump and repopulates the table when you want a newer controller snapshot. If Job Ready is pending, the request remains queued until normal-session initialization completes.
...
- Interpolation: `bilinear` is the conservative default for height-map compensation; `bicubic` remains available as an advanced option.
- Max correction (mm): hard limit for the largest absolute Z correction in the probed height map. Auto-Level apply fails closed if the map exceeds it.
- Max Z span (mm): hard limit for total probed map Z range. Auto-Level apply fails closed if the map exceeds it.
- Apply safety: Auto-Level apply rejects `G53` machine-coordinate motion, preserves `G43` / `G43.1` / `G49` TLO commands without compensation, rejects motion outside the probed X/Y bounds, and rejects moves that need a skipped or missing probe node.
- Recommendation: keep bilinear and the default safety limits unless you have a measured reason to adjust them for a known-safe setup.
