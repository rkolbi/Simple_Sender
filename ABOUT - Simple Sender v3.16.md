...
Recent release work included a deliberate truthfulness pass around validation wording, shipped workflows, settings, and lower jog-row polish. The current `3.16` release-facing docs record the latest full local release-gate validation while keeping hardware and cross-platform limitations explicit. The repository also includes a dedicated `MACHINE_VALIDATION_CHECKLIST.md` for the remaining reconnect, probing/modal-restore, accessory, popup, and Pi/Openbox validation work that automated tests cannot fully prove on their own.
