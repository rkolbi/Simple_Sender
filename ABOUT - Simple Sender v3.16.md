...
This document summarizes the current stable `3.16` release baseline.

**Simple Sender v3.16** is a GRBL 1.1h g-code sender built for practical, real-world CNC use. Rather than focusing on flashy presentation or unnecessary complexity, it is designed to provide a dependable, operator-friendly workflow that works well in an actual shop environment.
...
Recent release work included a deliberate truthfulness pass around validation wording, shipped workflows, settings, and lower jog-row polish. For the `3.16` version-label update, the release-facing docs distinguish the earlier full-suite validation snapshot from the targeted checks rerun for this label change. The repository also includes a dedicated `MACHINE_VALIDATION_CHECKLIST.md` for the remaining reconnect, probing/modal-restore, accessory, popup, and Pi/Openbox validation work that automated tests cannot fully prove on their own.
