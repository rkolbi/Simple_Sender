...
- The readiness popup no longer reappears for the same clean-session identity after the operator has acknowledged **OK / I’ll Home Later** or after normal-session homing has started from the workflow-row **Home** button.
- Startup/post-connect `$$` settings refresh now runs before the readiness popup during the normal position-verification phase instead of waiting for Job Ready or requiring a manual GRBL Settings refresh. This preserves `$130/$131/$132` travel data for bounded joystick hold jogging when the operator chooses **OK / I’ll Home Later**.

### Diagnostics
- Connection diagnostics now record serial/startup ownership, generation and object identity, synchronization request progress, position provenance, snapshot approval/installation acknowledgement, and the final Job Ready transition. Diagnostic bundles include the bounded authoritative worker connection state.
- Runtime diagnostics now include a bounded controller TX-origin tail for non-status line transmissions, including command origin, admission class, generation, epochs, normal-session phase, action identity, and transmit timestamp.

### Documentation
- README and the in-app About/help text now state explicitly that **OK / I’ll Home Later** only dismisses the reminder and that the CNC should be homed before jogging, zeroing, probing, parking, running jobs, or other movement actions, except for the homing action itself.
- Updated troubleshooting/FAQ wording for startup `$$` capture so it matches the current automatic settings-refresh behavior.
...
### Safety
- Added worker-owned, generation/serial/stream/recovery-bound controller suspension state. Pause, external Hold, and Door now close all ordinary and tool-change-macro TX immediately; Resume reopens controller admission only after explicit operator action and current-session confirmation. A pending tool-change workflow remains independently paused after that confirmation.
- Safety-significant realtime write failures during active or uncertain execution now synchronously enter Recovery Required before failure is returned. Current-session jog-cancel failure is always treated as uncertain, including when cached status appears Idle or no queued jog evidence remains.
- Streamed Tool Change commands and completion now carry one exact opaque connection/serial/stream/recovery/source/directive/request identity through UI handoff, macro execution, queueing, RX reservation, final serial dispatch, and completion. Stale or equal-but-reconstructed workflow identities cannot command or release a later directive. If a current replacement directive reaches the UI behind a still-live stale workflow thread, the stale thread is signaled and the replacement is explicitly failed with a Stop/restart instruction instead of being silently discarded.
- Auto-Level now uses an exclusive worker-owned lifecycle lease with complete queue/reservation/reset/jog/session busy accounting. Successful restoration creates an exact single-use installation ticket; the worker retains admission ownership while the UI stages the map, then atomically revalidates generation, serial, stream/recovery/source/coordinate epochs, alarm, trust, and restoration before publishing map fields and enabling Apply/Save. Installed maps retain exact provenance and fail closed on reset, alarm, recovery, session/source replacement, and relevant WCS/G92/TLO context changes. Historical loaded maps remain inactive. Cancel, window close, Escape, stale finalization, and failed restoration cannot publish success and retire ownership through the existing safe paths.
- Resume From now preserves the physical value of active `G43.1` across G20/G21 transitions and blocks ambiguous unit/TLO reconstruction, including multiple `G43.1`/`G49` or associated Z words in one block.

### Interface and documentation
- Added truthful Pause requested, Hold confirmed, External Hold, Door suspended, Resume requested, and Running confirmed projections.
- Console filters now display `All`, `Errors`, and `Alarms`.
- Synchronized the runtime, About, README, and release documentation to version `3.19.1` and retained the physical E-stop/power-isolation warning.

### Validation
- Current Windows / Python `3.12.1` direct validation as of `2026-07-16`: three consecutive full pytest runs passed with `2571 passed, 1 skipped` each (the environment-gated physical Kasa test); the repo Ruff, compileall, mypy-manifest, configured mypy, and direct changed-file mypy gates passed.
- Hardware-in-the-loop behavior, physical safety-door wiring, probe stopping distance, accessory power isolation, and emergency-stop behavior still require validation on the target machine.

## [3.19] - 2026-07-04

### Changed
- Runtime package metadata now reports `3.19`.
- Current release-facing docs and the in-app About title now identify the active application baseline as `3.19`.
- Auto-Level interpolation defaults now use bilinear as the conservative height-map method, with bicubic still available as an advanced option.

### Fixed
- Active-stream execution uncertainty is now latched in one worker-owned recovery-required state. A streamed `error:`, unexpected GRBL startup banner, or current-session connection loss invalidates the stream generation, quarantines acknowledgment/RX-buffer accounting, blocks further machine commands and workflows, and attempts feed hold followed by soft reset when the connection can still be written. Reports from a provably retired connection generation are discarded without altering its replacement.
- Reconnect recovery no longer assumes `last_acked_index + 1` is safe. The worker records the uncertain transmitted-command range, automatic reconnect/resume and one-click resume are disabled, and the operator must explicitly reset/reconnect and re-establish position, modal/setup, WCS/G92, TLO, spindle, and coolant state.
- Serial reader, transmitter, status, and UI events now carry a connection generation so callbacks from a retired connection cannot acknowledge, alter, or close its replacement. Reset-write failure remains visibly recovery-required and warns that buffered machine execution may still be continuing.
- Phase 0 command admission now serializes recovery latching with queue commits, Cycle Start, and serial writes; queued work and machine-state events carry stream/recovery ownership, active or final-ACK-pending disconnects fail closed, and reconnect is refused while a retired worker thread can still affect shared state.
- Reset confirmation is now distinct from machine recovery: the matching banner enters a state-untrusted phase, ordinary commands remain blocked, controller reports restore only explicitly requested reportable state, and an operator must home or accept position, verify accessories, and complete recovery before Ready operation. Final stream acknowledgment also remains execution-pending until a current-session Idle report confirms physical completion.
- Repeated reset banners, recovery-time alarms, and replacement connections now retire the prior recovery identity and all collected evidence. Recovery synchronization requires complete modal groups, rejects contradictory groups or F/S/T values, rejects non-finite controller numeric evidence and derived coordinates, and binds the accepted `$G`/`$#` values to the current request identity. The fixed incremental-arc behavior of supported GRBL firmware is recorded as an explicit request-bound inference when `$G` omits that mode.
- G-code replacement, clear, and reconnect restoration now use worker-authoritative reserve/install/commit transactions. Reserved sources are unrunnable; filename, preview, metadata, line counts, cache, and deferred UI stages revalidate the exact source/connection/stream/recovery/UI generation before worker commit. Failure or cancellation aborts into a cleared/unrunnable source, and older callbacks cannot overwrite or stream after a newer commit.
- Recovery Required now forces a reachable Recover action independently of ordinary toolbar locks. The recovery dialog includes an identity-bound serial Disconnect action with an explicit warning that disconnect is not an emergency stop and buffered controller work may continue.
- First clean connections and reconnects after clean idle disconnects retain the normal expected-banner startup path. Only replacement connections carrying unresolved execution uncertainty are rebased into the strict recovery workflow.
- Clean connections now distinguish banner-owned **Communication Ready** from **Job Ready**. Each new generation performs fresh normal-session `$G`/`$#` synchronization and requires current position plus homing or explicit physical-position acceptance; conservative M5/M9 evidence is request-bound, prior-generation trust is never inherited, and Run reports the same missing prerequisites enforced by the worker.
- Startup-banner ownership is bound to the exact generation and serial object with explicit pending, consumed, timed-out, and retired outcomes. Timeout commits only after atomically revalidating that exact owner, so a consumed banner cannot subsequently disconnect the session; status frames received first remain telemetry-only, early Run attempts cannot retire banner ownership, and any later same-generation banner is treated as a controller reset.
- Clean-session Job Ready now requires a worker-approved immutable `$G`/`$#`/position snapshot to be installed into the UI, DRO, offset, TLO, modal, and macro caches and acknowledged using the exact snapshot object. Failure, duplicate/stale acknowledgement, disconnect, reset, or replacement retires the handoff without enabling machine-driving controls or retaining prior-session cache trust.
- Recovery coordinate parsing now recognizes known modal, status, WCS, G28/G30/G92, TLO, and PRB report ownership before validating the complete envelope. Extra/missing fields, empty components, trailing garbage, non-finite values, numeric overflow, and malformed duplicates invalidate and clear the entire owning synchronization transaction, so a later `ok` cannot approve earlier evidence.
- Run and Resume From now require an exact UI/worker source identity in worker `COMMITTED` state with no pending reservation or installation transaction, in addition to current trust and execution-idle prerequisites. Deferred or stale UI callbacks cannot make a Reserved source runnable.
- If approved recovery snapshot installation fails, ordinary admission remains closed and the Recover action offers identity-bound **Retry Recovery Finalization** using the same immutable approved snapshot. A stale retry cannot affect a replacement recovery.
- Asynchronous Kasa outlet results now carry connection, stream, recovery, source, and command identities. Recovery submits immediate delay-bypassing dominant OFF work directly from the worker transition, records requested/confirmed/failed-unknown separately, and rejects stale delayed callbacks or results without allowing them to overwrite a newer recovery OFF.
- GRBL `error:` responses received after the final acknowledgment now retire the execution-pending completion identity and enter Recovery Required before a later Idle or queued UI completion can report Done or start post-job actions.
- Normal-session and recovery status evidence now rejects duplicate `MPos`, `WPos`, or `WCO` fields and rejects inconsistent simultaneous coordinate representations instead of trusting the last field value.
- Execution quarantine now retires pending confirmed Kasa stream directives. Recovery OFF dominance is installed before task submission and remains as a `failed_unknown` tombstone after queue, shutdown, executor, or task-creation rejection so older queued ON work stays suppressed; physical OFF delivery is still not implied.
- Kasa barriers, ownership, result staleness, and recovery tombstones now use canonical stable device-ID plus outlet identity across hostname/IP aliases, cache replacement, and rediscovery. Unknown aliases share a conservative provisional domain until atomic resolution/ownership merging. The exact dispatch commitment is recorded under the router state lock while holding the canonical outlet barrier, before the network call: older ON work not yet committed is suppressed, while an already committed call cannot be canceled, remains logically superseded, and still requires an OFF retry. Process-level physical outlet dispatch ownership now outlives a single router instance while a committed call is active, router shutdown reports incomplete if dispatch has not drained, and replacement routers serialize against unresolved older commitments. Confirmed Recovery OFF dominance is retired only after successful recovery completion for the exact identity. Failed OFF submission stays `failed_unknown`, remains dominant until an identity-valid OFF retry succeeds, and does not imply physical OFF.
- Kasa Recovery OFF tombstones are now status-dominant across later recovery epochs: `requested` and `failed_unknown` block ordinary ON through reconnects and replacement routers until an identity-valid OFF retry succeeds and the resulting `confirmed` tombstone is retired through the exact recovery-completion transition. The process-level dispatch registry now records unique router-instance ownership, drains only the router being shut down, and removes idle entries only after no router reference, active commitment, or unresolved/unretired tombstone remains.
- Ordinary post-Job-Ready status telemetry now shares recovery's strict three-axis coordinate parser, normalizes recognized coordinate field names, and tracks raw status separately from the last validated-and-installed coordinate signature. Malformed or inconsistent evidence cannot become a relaxed duplicate baseline, enter coordinate coalescing, advance the position throttle/freshness sequence, signal a coordinate waiter, or overwrite operational/macro coordinates; freshness is published only after the exact validated values are installed in operational, DRO, and macro caches.
- Deferred coordinate coalescing now stores immutable ownership for each pending coordinate update, including connection generation, recovery epoch, session/serial identity, validated coordinate evidence, coordinate signature, and request id. Pending callbacks are retired on disconnect, recovery entry, reset/recovery identity change, replacement connection, and shutdown; stale callbacks cannot install old or newer evidence or publish coordinate freshness.
- Added a deterministic completion-boundary matrix covering final ACK, controller Idle, completion preparation/publication, and UI acknowledgement against streamed error, alarm, unexpected startup banner, disconnect, and Stop Job ownership.
- Recovery snapshot installation now requires the exact worker-approved immutable snapshot object and exact single-use recovery finalization identity. Equal copies, stale retries, duplicate acknowledgements, resets, alarms, disconnects, and replacement sessions remain fail closed.
...
- Auto-Level apply now fails closed for `G53` machine-coordinate motion, outside-map job motion, excessive height-map correction/span, and moves that require a skipped or missing probe node.
- Auto-Level apply now preserves `G43`, `G43.1`, and `G49` tool-length-offset commands unchanged instead of rewriting them as compensated motion.
- Updated the public GitHub sync workflow to copy the current `v3.19` About and release-note files.

### Validation
- Current `3.19` direct validation on Windows / Python `3.12.1` as of `2026-07-04`: `.\.venv\Scripts\python.exe -m pytest` passed with `1981 passed, 2 skipped`; `.\.venv\Scripts\python.exe tools\run_ruff.py check .` passed; `.\.venv\Scripts\python.exe -m compileall -q simple_sender tests tools` passed; `.\.venv\Scripts\python.exe tools\check_mypy_targets.py --expected-count 141` passed; `.\.venv\Scripts\python.exe -m mypy --config-file mypy.ini` passed.
- The `run_tests.bat` wrapper, pytest coverage gate, critical-path coverage check, cross-platform CI, hardware-in-the-loop behavior, and Raspberry Pi image provenance/build validation were not rerun for this `3.19` release work.

### Documentation
- README and the in-app About/operator reference now describe execution-uncertainty recovery, the removal of reconnect-from-last-ACK, and the explicit reset/state-verification requirement.
- Release-note/About filenames now identify the `3.19` baseline.
- README, About, release notes, and in-app help now describe the Auto-Level fail-closed apply behavior, correction/span limits, TLO preservation, and bilinear default.
